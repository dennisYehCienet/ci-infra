...
resource "google_compute_instance" "buildkite-agent-instance" {
  ...
  name     = "dennis-test-vllm-ci-cpu-${count.index}"

  boot_disk {
    auto_delete = true
    device_name = "dennis-test-vllm-ci-cpu-${count.index}"
...
resource "google_compute_address" "static" {
  provider = google-beta
  name     = "dennis-test-vllm-ci-cpu-${count.index}-ip"
  count    = var.instance_count
}
